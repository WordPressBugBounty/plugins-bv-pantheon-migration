...
if (!class_exists('PTNRespStream')) :

	class PTNStream extends PTNCallbackBase {
		public $bvb64stream;
		public $bvb64cksize;
		public $checksum;
		...
		public static function startStream($account, $request) {
			...
			$stream = new PTNRespStream($request);
			if ($request->isAPICall()) {
				$stream = new PTNHttpStream($request);
				if (!$stream->connect()) {
					$apicallstatus = array(
...
class PTNRespStream extends PTNStream {
	public $bvboundry;
...
class PTNHttpStream extends PTNStream {
	var $user_agent = 'PTNHttpStream';
	var $host;
	var $port;
	var $timeout = 20;
	var $conn;
