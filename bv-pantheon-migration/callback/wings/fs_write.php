...
if (!class_exists('PTNFSWriteCallback')) :

class PTNFSWriteCallback extends PTNCallbackBase {
	...
	public function removeDirs($dirs) {
		...
		foreach ($dirs as $dir) {
			...
			if ((PTNWPFileSystem::getInstance()->isDir($dir) === true) && !is_link($dir)) {
				if ($this->isEmptyDir($dir)) {
					$dir_result['status'] = PTNWPFileSystem::getInstance()->rmdir($dir);
					if ($dir_result['status'] === false) {
						$dir_result['error'] = "RMDIR_FAILED";
						$fs_error = PTNWPFileSystem::getInstance()->checkForErrors();
						if (isset($fs_error)) {
	...
	public function doChmod($path_infos) {
		...
		foreach ($path_infos as $path => $mode) {
			...
			if (PTNWPFileSystem::getInstance()->exists($path) === true) {
				$path_result['status'] = PTNWPFileSystem::getInstance()->chmod($path, $mode);
	...
	public function renameFiles($path_infos) {
		...
		foreach ($path_infos as $oldpath => $newpath) {
			...
			if (PTNWPFileSystem::getInstance()->exists($oldpath)) {
				$action_result['status'] = PTNWPFileSystem::getInstance()->move($oldpath, $newpath, true);
	...
	public function writeContentToFile($content, $ofile) {
		...
		if (PTNWPFileSystem::getInstance()->putContents($ofile, $content) === false) {
			$result['error'] = 'UNABLE_TO_WRITE_TO_TMP_OFILE';
			$fs_error = PTNWPFileSystem::getInstance()->checkForErrors();
			if (isset($fs_error)) {
	...
	public function moveUploadedFile($ofile) {
		...
		if (isset($_FILES['myfile'])) {
			...
			if ($is_upload_ok) {
				...
				if (PTNWPFileSystem::getInstance()->move($tmp_name, $ofile, true) === false) {
	...
	public function process($request) {
		...
		case "fleops":
			...
			break;
		case "cncatfls":
			$bsize = (isset($params['bsize'])) ? $params['bsize'] : (8 * PTNFSWriteCallback::MEGABYTE);
