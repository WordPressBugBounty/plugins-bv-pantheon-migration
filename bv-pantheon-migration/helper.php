...
if (!class_exists('PTNHelper')) :
	class PTNHelper {
		...
		public static function fileRemovePattern($fname, $pattern, $is_regex = false) {
			...
			if (!PTNWPFileSystem::getInstance()->exists($fname)) {
				return;
			}

			$content = PTNWPFileSystem::getInstance()->getContents($fname);
			if ($content !== false) {
				if ($is_regex !== false) {
				...
				if ($content !== $modified_content) {
					PTNWPFileSystem::getInstance()->putContents($fname, $modified_content,
							PTNWPFileSystem::getInstance()->getchmodOctal($fname));
				}
			}
		}
